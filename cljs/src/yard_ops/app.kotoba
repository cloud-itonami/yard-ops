(ns yard-ops.app
  "yard-ops appview — reagent + re-frame, view built from jp-go-dds
  (デジタル庁デザインシステム) hiccup.

  This is a faithful port of the previous SvelteKit scaffold
  (`60-apps/etzhayyim-project-yard-ops/svelte/src/routes/+page.svelte`): a
  static info page describing this component surface itself — its
  title/project/name/kind, its declared route count and public routes, its
  declared runtime vars, whether xrpc is configured, and the source path of
  the page that renders all of this. The Svelte page never called the
  dispatcher or any dock/robot logic — it was pure static markup describing
  the surface's own metadata, and this port adds no yard-operations
  functionality that was not already there.

  The Svelte scaffold's `routes`/`vars` arrays were stale empty literals
  (`routeCount: 0`, `routes: []`, `vars: []`); this port corrects them from
  the component's actual `wrangler.jsonc` (`routes[].pattern` and the `vars`
  map keys), same as the landed reference this was copied from
  (`cloud-itonami/app-tia`'s `appview/tia-mcp-component/cljs`).

  Backend TypeScript is unaffected by this migration: `../src/app.ts` (the
  dispatcher/thin-edge Worker) is untouched. The one other TypeScript file
...
(def default-db
  {:title "Ai etzhayyim Project Yard Ops"
   :project "etzhayyim-project-yard-ops"
   :name "etzhayyim-project-yard-ops"
   :kind "cloudflare surface"
   :route-count 2
   :routes ["y4rd0p5x.etzhayyim.com/*" "yard-ops.etzhayyim.com/*"]
   ...
   :xrpc? true
   :relative-path "cljs/src/yard_ops/app.cljs"})
